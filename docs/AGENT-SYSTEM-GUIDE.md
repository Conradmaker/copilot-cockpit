...
- 공통 packet schema (`TASK`, `EXPECTED_OUTCOME`, `MUST_DO`, `MUST_NOT_DO`, `CONTEXT`, optional `ROLE`/`CURRENT_DATE`/`SEARCH_STRATEGY`/`SCOPE`/`EXECUTION_PLAN`, `task_packet`), phase별 산출물 정책, subagent 선택 기준의 기준 문서: `.github/instructions/subagent-invocation.instructions.md`
...
- 특히 Planning 인계의 `artifacts.md` discovery와 Execution/Review packet-only brief 정책은 사람이 읽는 문서와 runtime 문서에서 같은 의미로 설명되어야 한다.
...
- 사람이 읽는 문서가 Planning 단계의 생성된 산출물 인덱스, implementation/review packet의 packet-only digest 규칙과 충돌하지 않는다.
