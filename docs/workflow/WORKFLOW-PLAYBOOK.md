...
- Planning → Downstream Definition: 승인된 `prd.md`와 Coordinator 검토를 통과한 품질 관문이 모두 있어야 한다.
...
또 다른 자주 쓰는 경로는 Mate가 정리된 PRD 요약을 사용자에게 간단히 설명한 뒤 downstream mode를 `디자인만`, `기술설계만`, `둘 다` 중 하나로 auto-decision 하고, 그 판단에 따라 Designer, Architector, 또는 둘 다를 여는 것이다.
