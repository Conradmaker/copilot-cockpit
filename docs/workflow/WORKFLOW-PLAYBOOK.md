...
- Planning → Downstream Definition: 승인된 `prd.md`와 Coordinator `Scores`를 포함한 품질 관문이 모두 있어야 한다.
...
- Planning 단계의 기준 문서다.
- findings, Coordinator verdict, Coordinator Scores, PRD 구조가 크게 바뀔 때마다 갱신한다.
...
- `/memories/session/design.md`: 승인된 PRD를 바탕으로 시각, UX, 상호작용 결정을 확장한 문서다. 보통 Mate가 정리된 PRD 요약을 사용자에게 간단히 설명한 뒤 Designer를 열어 만든다.
- `/memories/session/technical.md`: 승인된 PRD를 바탕으로 architecture, integration, technical constraint를 확장한 문서다. 보통 Mate가 정리된 PRD 요약을 사용자에게 간단히 설명한 뒤 downstream mode에 따라 Architector를 열어 만든다.
...
5. Council 검토 지점
   - Mate는 작업 성격에 맞는 Coordinator 관점을 최소 2개 고른 뒤, 각 role을 분리된 Coordinator 호출로 동시에 병렬로 연다.
   - Planning PRD review의 Coordinator output에는 PRD Quality Gate dimension별 score와 total score가 포함되어야 한다.
...
8. 품질 관문과 PRD 승인
   - 품질 관문을 통과해야 한다.
   - 통과 기준은 최신 revision이 Coordinator 검토 상태이고 Coordinator `Scores`와 PRD `Quality Gate`의 total 88 이상이며 치명적 차단 요소가 없는 것이다.
   - 통과 뒤에는 정리된 PRD 요약을 사용자에게 간단히 설명한다.
...
- 승인된 `prd.md`가 있다.
- downstream mode가 결정되었거나 Execution 진입을 준비해야 한다.
- Planning 흐름이 무효화된 상태가 아니다.

자주 쓰는 경로 중 하나는 Mate가 정리된 PRD 요약과 coordinator signal을 바탕으로 design elaboration 필요성을 auto-decision 하고 Designer를 호출해 `design.md`를 만드는 것이다.
또 다른 자주 쓰는 경로는 Mate가 정리된 PRD 요약을 사용자에게 간단히 설명한 뒤 downstream mode를 `디자인만`, `기술설계만`, `둘 다` 중 하나로 auto-decision 하고, 그 판단에 따라 Designer, Architector, 또는 디자인 이후 기술설계 순서로 여는 것이다.
`둘 다`가 결정된 경우에는 Designer를 먼저 연 뒤 그 결과를 기준으로 Architector를 호출한다.
`기술설계만`이 결정됐거나 `둘 다`에서 Architector를 열 차례인데 technical seed가 약하거나 architecture ambiguity가 남아 있으면, Mate가 clarification 또는 research 흐름을 먼저 다시 열고 그 뒤 Architector를 호출한다.
...
- Coordinator `Scores`와 PRD `Quality Gate` total 88 이상
- 치명적 차단 요소 없음
- 최신 revision이 Coordinator 검토 상태여야 함
- 그 뒤 정리된 PRD 요약과 downstream auto-decision으로 자연스럽게 이어질 수 있어야 함
