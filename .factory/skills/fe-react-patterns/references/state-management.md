...
React 컴포넌트의 상태를 효과적으로 관리하고 공유하기 위한 패턴들을 정리한 문서예요.
Provider를 활용한 상태 끌어올리기, 구현 분리, 제네릭 Context 인터페이스 설계를 다뤄요.
...
상태 관리를 전용 Provider 컴포넌트로 끌어올려요.
이렇게 하면 메인 UI 바깥에 있는 형제 컴포넌트도 prop drilling이나 ref 없이 상태에 접근할 수 있어요.
...
**핵심 인사이트:** 공유 상태가 필요한 컴포넌트들이 시각적으로 서로 안에 중첩될 필요는 없어요.
같은 Provider 안에만 있으면 돼요.
...
Provider 컴포넌트가 상태 관리 방법을 아는 **유일한 곳**이어야 해요.
UI 컴포넌트는 Context 인터페이스만 소비하고, 상태가 useState에서 오는지, Zustand에서 오는지, 서버 동기화에서 오는지 알 필요 없어요.
...
같은 `Composer.Input` 컴포넌트가 두 Provider 모두에서 동작해요.
Context 인터페이스에만 의존하고, 구현에는 의존하지 않기 때문이에요.
...
컴포넌트 Context에 **state, actions, meta** 세 부분으로 구성된 **제네릭 인터페이스**를 정의해요.
이 인터페이스는 어떤 Provider든 구현할 수 있는 계약(contract)이에요.
같은 UI 컴포넌트가 완전히 다른 상태 구현과 함께 동작할 수 있게 해요.

**핵심 원칙:** 상태를 끌어올리고, 내부를 조합하고, 상태를 의존성 주입 가능하게 만들어요.
...
Provider 경계가 중요해요 — 시각적 중첩이 아니에요.
`Composer.Frame` 바깥에 있는 컴포넌트도 Provider 안에만 있으면 상태와 액션에 접근할 수 있어요.
...
**요약:** UI는 조합해서 사용하는 재사용 가능한 조각이에요. 상태는 Provider가 의존성 주입해요. Provider를 바꾸면 UI는 그대로 유지돼요.
