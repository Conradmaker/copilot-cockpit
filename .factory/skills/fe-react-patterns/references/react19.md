...
React 19에서 도입된 주요 API 변경사항과 마이그레이션 가이드예요.

> ⚠️ **React 19 이상에서만 적용돼요.** React 18 이하를 사용하고 있다면 이 문서를 건너뛰세요.
...
React 19에서 `ref`가 일반 prop이 되었어요. 더 이상 `forwardRef` 래퍼가 필요 없어요.
컴포넌트 정의가 더 깔끔해지고, 타입도 단순해져요.
...
`use()`는 `useContext()`를 대체하는 새로운 API예요.
가장 큰 차이점은 `use()`가 **조건부로 호출**될 수 있다는 거예요. `useContext()`는 훅 규칙(Rules of Hooks)에 따라 항상 컴포넌트 최상위에서만 호출 가능했지만, `use()`는 조건문이나 반복문 안에서도 사용할 수 있어요.
...
React 19에서 Context Provider도 간소화되었어요:
...
- `use()`는 React 19에서만 사용 가능해요. 프로젝트의 React 버전을 먼저 확인하세요.
- `forwardRef`는 React 19에서 deprecated이지만 아직 동작해요. 점진적으로 마이그레이션하세요.
- Context Provider의 새 문법은 React 19에서만 동작해요.
