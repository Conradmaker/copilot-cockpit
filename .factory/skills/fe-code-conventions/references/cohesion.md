...
> 수정되어야 할 코드가 항상 같이 수정되는지를 말해요. 함께 수정되어야 할 부분이 반드시 함께 수정되도록 구조적으로 뒷받침하는 것이 중요해요.
...
프로젝트에서 Hook, 컴포넌트, 유틸리티 함수 등을 여러 파일로 나누어 관리할 때, 함께 수정되는 소스 파일을 하나의 디렉토리에 배치하면 코드의 의존 관계를 명확하게 드러낼 수 있어요.
...
- 특정 기능 삭제 시 연관된 코드가 함께 삭제되지 못해 사용되지 않는 코드가 남을 수 있어요
- 프로젝트 규모가 커지면 디렉토리 하나에 100개가 넘는 파일이 쌓일 수 있어요
...
매직 넘버가 코드에 직접 사용되면, 관련 코드를 수정할 때 조용히 서비스가 깨질 수 있는 위험성이 있어요.
...
`300`이 애니메이션 완료 대기 시간이라면, 애니메이션을 변경했을 때 이 값도 함께 수정되어야 해요. 하지만 매직 넘버로 사용되면 수정이 누락될 수 있어요 — **같이 수정되어야 할 코드 중 한쪽만 수정되는 것**이에요.
...
상수로 선언하면, 애니메이션 관련 코드를 수정할 때 이 상수도 함께 확인하게 돼요.

---

## 예제 3: 폼의 응집도 전략

폼을 관리할 때는 **필드 단위 응집도**와 **폼 전체 단위 응집도** 중 상황에 맞는 방식을 선택해야 해요.

### 전략 A: 필드 단위 응집도

각 필드가 독립적으로 검증 로직을 가지는 방식이에요.
