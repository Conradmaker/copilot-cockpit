...
> 코드가 읽기 쉬운 정도를 말한다. 코드를 변경하기 쉽게 만들려면 먼저 코드가 어떤 동작을 하는지 이해할 수 있어야 한다.
...
상수 `ANIMATION_DELAY_MS`로 선언해 맥락을 명확하게 표시한다.

---

## 예제 2: 복잡한 조건에 이름 붙이기

복잡한 조건식이 이름 없이 사용되면, 조건이 뜻하는 바를 한눈에 파악하기 어렵다.
...
삼항 연산자를 복잡하게 사용하면 조건의 구조가 명확하게 보이지 않아 코드를 읽기 어려울 수 있다.
...
범위를 확인하는 조건문에서 부등호의 순서가 자연스럽지 않으면, 조건의 의도를 파악하는 데 시간이 더 걸린다.
...
중간값 `a`를 두 번 확인해야 하는 인지적 부담이 있다.
...
`80 ≤ score ≤ 100`처럼 수학의 부등식과 같은 형태로 읽혀서, 범위 조건을 직관적으로 이해할 수 있다.

---

## 예제 5: 구현 상세 추상화하기

한 사람이 코드를 읽을 때 동시에 고려할 수 있는 맥락의 숫자는 제한되어 있다 (약 6~7개). 불필요한 맥락은 추상화해 읽기 쉽게 만든다.
...
로그인 확인 로직이 추상화 없이 노출되어 있어서, 한 번에 이해해야 하는 맥락이 많다.
...
로그인 확인/이동 로직을 `AuthGuard`로 분리해 각 컴포넌트의 맥락을 줄인다.
...
2가지 권한 상태를 하나의 컴포넌트에서 처리하여, 동시에 실행되지 않는 코드가 교차되어 나타난다.
...
- 분기가 단 하나로 합쳐지고
- 각 컴포넌트에서 하나의 분기만 관리하여 맥락이 줄어든다

---

## 예제 7: 시점 이동 줄이기

코드를 읽을 때 위아래를 오가거나, 여러 파일/함수/변수를 넘나들면서 읽는 것을 **시점 이동**이라고 한다. 시점 이동이 많을수록 코드 파악에 시간이 더 걸린다.
...
여러 차례의 시점 이동 없이, 컴포넌트만 보면 조건을 한눈에 파악할 수 있다.

---

## 예제 8: 로직 종류에 따라 합쳐진 함수 쪼개기

쿼리 파라미터, 상태, API 호출과 같은 로직의 종류에 따라 함수/Hook을 만들면, 한 번에 다루는 맥락이 많아져서 이해하기 힘들고 수정하기 어려운 코드가 된다.

모놀리식 Hook(예: `usePageState`)을 각 쿼리 파라미터별로 별도의 Hook으로 쪼개면, 명확한 이름과 책임을 가지고 수정 시 영향 범위가 좁아진다.

이 예제는 결합도 관점에서도 중요하다. 자세한 내용은 coupling.md 예제 1을 참고한다.
