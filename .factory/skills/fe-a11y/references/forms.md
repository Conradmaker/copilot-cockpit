...
입력 폼과 버튼 등 인터랙티브 요소의 접근성 패턴을 다뤄요.

---

## `<form>` 태그 사용하기

입력 요소는 반드시 `<form>`으로 감싸야 해요. `<form>`을 사용하면 별도의 스크립트 없이 다음 기능이 제공돼요:
...
<form onsubmit="event.preventDefault(); /* 원하는 동작 */">
  ...
  <button type="submit">로그인</button>
  <button type="button">취소</button>  <!-- 단순 동작용 -->
</form>
```
...
> ⚠️ `<form>` 안의 `<button>`은 기본적으로 submit 동작을 해요. 단순 클릭 이벤트만 필요한 버튼은 `type="button"`을 꼭 명시하세요.
...
> 삭제 버튼에 `tabindex="-1"`을 지정하여 키보드 포커스를 받지 않게 했어요. 텍스트는 `Backspace` 키로 지울 수 있기 때문에 키보드 사용자에게 불필요한 포커스 이동을 방지해요.

---

## 가짜 버튼 금지

`<div>`나 `<span>`에 `cursor: pointer`와 `onClick`만 추가한 것은 진짜 버튼이 아니에요.
...
[React-Aria](https://react-spectrum.adobe.com/react-aria/index.html)의 `useButton` 훅을 사용하면 접근성 속성을 직접 처리하지 않아도 돼요.
...
> `useButton`이 `role`, `tabIndex`, `onKeyDown` 등 필수 접근성 설정을 모두 제공해요.

### 링크에도 동일하게 적용

링크가 가능한 요소는 반드시 `<a>` 태그를 사용하세요.
- `<a>` 태그는 block 요소를 자식으로 포함할 수 있어요
