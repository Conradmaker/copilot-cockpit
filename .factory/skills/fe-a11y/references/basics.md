...
스크린 리더가 화면 요소를 올바르게 전달하기 위해 알아야 할 3가지 핵심 요소를 다뤄요.

## 역할(Role)

UI 컴포넌트가 어떤 의미를 지니는지 스크린 리더가 이해하게 하려면 `role` 속성을 명확히 지정해야 해요.
...
- HTML 기본 요소(`<button>`, `<input>`, `<a>` 등)는 이미 역할이 내장되어 있어요
- `<div>`로 커스텀 요소를 만들거나, 탭(tab), 스위치(switch)처럼 기본 요소가 없을 때 명시적으로 `role`을 선언해야 해요
...
레이블은 화면에 보이지 않는 텍스트를 스크린 리더에 전달해요. 특히 인터랙티브 요소(입력창, 버튼 등)에 텍스트가 없으면 중요한 상호작용을 놓칠 수 있어요.

### 레이블 우선순위

스크린 리더는 다음 순서대로 이름을 읽어요:
...
```tsx
// 여러 ID를 공백으로 연결하면 순서대로 이어 붙여서 읽어요
...
컴포넌트의 동작 변화(켜짐/꺼짐, 펼침/접힘 등)를 스크린 리더에 알려주려면 상태 속성을 설정해야 해요.
...
> **참고:** `role="alert"` = `aria-live="assertive"`, `role="status"` = `aria-live="polite"`와 동일하게 동작해요.
