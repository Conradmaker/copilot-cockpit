...
불필요한 리렌더를 줄이고, 상태 관리를 최적화하는 12개 규칙이에요.

> **참고:** 프로젝트에 [React Compiler](https://react.dev/learn/react-compiler)가 활성화되어 있다면, `memo()`, `useMemo()`, `useCallback()`의 수동 메모이제이션이 자동화돼요. 하지만 함수형 setState, 파생 상태 등의 패턴은 여전히 수동 적용이 필요해요.

---

## 1. 단순 표현식에 useMemo 사용 금지

결과가 원시 타입(boolean, number, string)인 간단한 표현식은 `useMemo`로 감싸지 마세요. `useMemo` 호출과 의존성 비교가 표현식 자체보다 비용이 클 수 있어요.
...
비싼 연산을 메모이즈된 컴포넌트로 추출하면, 로딩 상태 등에서 조기 반환이 가능해요.
...
메모이즈된 컴포넌트의 비원시 타입 기본값(배열, 함수, 객체)은 매 리렌더마다 새 인스턴스가 생성되어 메모이제이션이 깨져요.
...
현재 props/state에서 계산할 수 있는 값은 상태에 저장하거나 Effect에서 업데이트하지 마세요. 렌더링 중 직접 계산하면 추가 리렌더와 상태 동기화 문제를 피할 수 있어요.
...
연속적으로 변하는 값 대신 파생된 boolean 상태를 구독하면 리렌더 빈도를 줄일 수 있어요.
...
상태를 현재 값 기반으로 업데이트할 때 함수형 업데이트를 사용하면, stale closure를 방지하고 안정적인 콜백 참조를 만들 수 있어요.
...
비용이 큰 초기값은 함수 형태로 `useState`에 전달하세요. 함수 형태가 아니면 매 렌더마다 초기화 코드가 실행돼요.
...
빈번하고 비긴급한 상태 업데이트를 transition으로 표시하면 UI 반응성을 유지할 수 있어요.
...
동적 상태(searchParams, localStorage)를 콜백 내에서만 읽는다면, 구독하지 말고 필요할 때 직접 읽으세요.
...
특정 사용자 액션(submit, click)에 의해 트리거되는 부수 효과는 해당 이벤트 핸들러에서 실행하세요. state + effect 패턴으로 모델링하면 관련 없는 변경에도 Effect가 재실행돼요.
...
자주 변하지만 리렌더가 필요 없는 값(마우스 트래커, 인터벌, 일시적 플래그)은 `useRef`에 저장하세요.
