...
번들 크기는 초기 로딩 속도에 직접적으로 영향을 미쳐요. barrel import 하나가 200~800ms 비용을 발생시킬 수 있어요.

---

## 1. Barrel 파일 Import 회피

barrel 파일(index.js에서 `export * from './module'`)을 통한 import는 수천 개의 미사용 모듈을 로드해요.

**왜 tree-shaking이 도움이 안 되는가:** 라이브러리가 external로 처리되면 번들러가 최적화할 수 없고, 번들에 포함하면 빌드가 크게 느려져요.
...
초기 렌더에 필요하지 않은 무거운 컴포넌트는 `next/dynamic`으로 지연 로드하세요.
...
분석, 로깅, 에러 트래킹은 사용자 인터랙션을 차단하지 않아요. 하이드레이션 후에 로드하세요.
...
무거운 번들을 필요하기 전에 미리 로드하여 체감 지연을 줄이세요.
...
`typeof window !== 'undefined'` 체크는 SSR에서 preload 모듈의 번들링을 방지해요.
