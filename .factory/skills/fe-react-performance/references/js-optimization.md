...
JavaScript 성능 최적화 6개 규칙과 고급 패턴 3개를 포함해요.
비동기 병렬 처리(`Promise.all`)는 [async-waterfall.md](async-waterfall.md)를 참고하세요.
...
동일한 키로 여러 번 `.find()`나 `.includes()`를 호출하면 Map/Set으로 인덱스를 만드세요.
...
결과가 결정되면 즉시 반환하여 불필요한 처리를 건너뛰세요.
...
동일한 입력으로 반복 호출되는 함수는 모듈 레벨 Map으로 결과를 캐싱하세요.
...
Map(hook 아닌)을 사용하면 유틸리티, 이벤트 핸들러 등 어디서든 동작해요.

---

### 4. RegExp를 루프 밖으로 호이스팅

렌더 내에서 RegExp를 생성하지 마세요. 모듈 스코프로 호이스팅하거나 `useMemo()`로 메모이즈하세요.
...
**⚠️ 글로벌 정규식은 mutable `lastIndex` 상태가 있어요:**
...
스타일 쓰기와 레이아웃 읽기를 번갈아 하면 동기 리플로우가 강제돼요. 쓰기를 모아서 하고, 읽기는 한 번에 하세요.
...
인라인 스타일보다 CSS 클래스를 선호하세요. 브라우저가 캐싱하고 유지보수가 쉬워요.

---

### 6. toSorted()로 불변 정렬

`.sort()`는 배열을 직접 변이하여 React 상태/props 버그를 유발해요. `.toSorted()`를 사용하세요.
...
앱 전체 초기화를 `useEffect([])`에 넣지 마세요. 컴포넌트가 리마운트되면 Effect가 재실행돼요. 모듈 레벨 가드를 사용하세요.
...
콜백이 변경될 때 Effect가 재구독하는 것을 방지하려면 ref에 저장하세요.
...
의존성 배열에 추가하지 않고도 최신 값에 접근할 수 있어요. stale closure를 방지하면서 Effect 재실행을 막아요.
