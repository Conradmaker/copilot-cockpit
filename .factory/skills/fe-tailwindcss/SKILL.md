---
name: fe-tailwindcss
description: "Tailwind CSS v4 styling patterns and conventions for frontend applications. Use this skill when writing component styles with Tailwind, creating layouts (stacks, grids, centering), handling className merging with cn()/tailwind-merge, defining variants with CVA, implementing dark mode, adding animations/transitions, setting up responsive design, configuring @theme tokens, or migrating from Tailwind v3 to v4. Always consult this skill when writing any Tailwind CSS code, even for simple styling tasks. For UI element component API design (as/asChild, prop typing) use fe-ui-element-components. For React component architecture use fe-react-patterns. For accessibility use fe-a11y. Triggers on: Tailwind CSS, className, cn(), tailwind-merge, cva, class-variance-authority, @theme, @utility, @custom-variant, dark mode styling, responsive classes, animation utilities, design tokens, OKLCH, v-stack, h-stack, gap, layout utilities, 테일윈드, 클래스네임 병합, 스타일링, 다크모드, 반응형, 애니메이션, 트랜지션, 레이아웃 유틸리티, 디자인 토큰, 색상 토큰, CVA, variant 스타일."
---
