...
- Coordinator는 coord-types/{type}.md를 동적 로드해 type-specific 검토를 수행한다.
- quality gate 통과 후 approved plan briefing을 보여주고, askQuestions로 mode를 확인한 뒤 handoff.md를 작성한다. handoff.md가 완성된 뒤에만 handoff surface를 노출한다.
...
| 에이전트 | 왜 이 역할이 필요한가 |
| --- | --- |
| [.github/agents/Mate.agent.md](.github/agents/Mate.agent.md) | EARS requirements와 다차원 커버리지로 spec-first plan을 고정하고 planning loop를 반복해 premature execution을 막는다. |
...
| [.github/agents/Coordinator.agent.md](.github/agents/Coordinator.agent.md) | coord-types/{type}.md를 동적 로드해 type-specific planning lane과 milestone의 fidelity를 검증해 plan drift와 verification gap을 드러낸다. |
...
| [.github/skills/brainstorming/SKILL.md](.github/skills/brainstorming/SKILL.md) | 창의적/아키텍처 작업 전 빠르고 집중된 의사결정으로 방향을 잡게 만든다. |
...
| [.github/skills/ds-visual-design/SKILL.md](.github/skills/ds-visual-design/SKILL.md) | 색상, 타이포그래피, 스페이싱, 시각적 계층, 깊이감 같은 visual design 기초를 체계화한다. |
| [.github/skills/ds-ui-patterns/SKILL.md](.github/skills/ds-ui-patterns/SKILL.md) | SaaS 레이아웃, 대시보드, 참여형 UI, 모션, AI UI 패턴 같은 현대 UI 구조 결정을 돕는다. |
...
| [.github/skills/refero-design/SKILL.md](.github/skills/refero-design/SKILL.md) | 새 화면, 플로우, 인터페이스 설계 시 Refero MCP로 실제 제품을 연구해 근거 기반 디자인을 유도하고, 'AI 슬롭' 디자인을 방지한다. |
