...
- quality gate 통과 후 approved plan briefing을 보여주고, askQuestions로 mode를 확인한다. 응답으로 handoff 실행 허용 상태가 성립하면 확인된 mode 기반으로 handoff.md를 작성한다.
...
- 두 모드 모두 구현 방향에 대한 확신이 흔들리거나 drift가 의심될 때 Coordinator에 롤을 지정해 리뷰를 요청할 수 있다.
...
| [.github/agents/Coordinator.agent.md](.github/agents/Coordinator.agent.md) | coord-roles/{role}.md를 동적 로드해 role-specific planning lane과 실행 중 리뷰를 통해 plan drift와 verification gap을 드러낸다. |
