...
- README, workflow playbook, migration guide 같은 human-facing 문서는 규칙을 요약하거나 중복 서술해도 되지만, exact field definition과 completeness 기준의 final owner는 `instructions/` 또는 relevant `.agent.md` 한 곳에만 둔다.
...
- human-facing docs의 reader-oriented duplication은 허용하되, 행동 규칙의 exact owner는 하나로 잠근다.
