...
- current execution context를 기반으로 execution plan을 처음 만들 때
...
- execution plan은 implementation task 구조와 review strategy를 함께 담아야 한다.
...
### 6.1 Reviewer Role Activation

| Reviewer role | Mandatory or optional | Trigger | Scope |
|---------------|-----------------------|---------|-------|
| security | {mandatory/optional} | {예: auth, user input, persistence 변경} | {파일 또는 기능 범위} |
| frontend | {mandatory/optional} | {예: React state/render/shared UI 변경} | {범위} |
| design | {mandatory/optional} | {예: UX/UI, copy, responsive surface 변경} | {범위} |
| performance | {mandatory/optional} | {예: hot path, bundle, data access risk} | {범위} |
| code-quality | {mandatory/optional} | {예: refactor, maintainability, error handling risk} | {범위} |

### 6.2 Parallel Review Waves

| Review Wave | Reviewer roles | Can Start When | Notes |
|-------------|----------------|----------------|-------|
| R1 | {security, code-quality} | {예: implementation complete} | {왜 이 wave가 필요한가} |
| R2 | {optional roles} | {trigger 충족 시} | {scope/overlap 주의점} |

### 6.3 Final Board Gate

- final reviewer role: `board`
...
| Reviewer role | Rework owner | Notes |
|---------------|--------------|-------|
| {role} | {Commander / implementer} | {follow-up 기준} |
...
- Review Strategy에 mandatory reviewer, optional role trigger, final `board` gate가 있는가
- gotcha/risk가 식별되었는가
