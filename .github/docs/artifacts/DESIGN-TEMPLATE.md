...
- motion duration과 easing은 구체적으로 잠근다. ease-out, ease-in-out 같은 generic 키워드로 끝내지 않는다. 예: 0.6s cubic-bezier(0.16, 1, 0.3, 1).
...
- Display scale and size range: {hero, section, body, meta의 exact px 또는 rem으로 잠근다 — 예: hero 72px / section 48px / body 16px / meta 12px. size band 형용사로 끝내지 않는다.}
- Weight hierarchy: {header / section / body / meta의 font-weight 수치로 잠근다 — 예: display 700, section 600, body 400}
- Letter spacing and rhythm: {heading, body, label 각각의 em 단위 tracking 값 — 예: heading −0.02em, body 0em, label 0.05em. tight/generous 형용사로 끝내지 않는다.}
- Leading strategy: {display, section, body의 구체적인 line-height 숫자 — 예: display 1.05, section 1.2, body 1.6. 형용사 리듬 묘사로 끝내지 않는다.}
- Metadata or label treatment: {mono 여부, uppercase 여부, 구체적 tracking 수치 — 형용사 없이 값으로 잠근다}

## 5. Layout Grammar & Spatial Rules

- Primary grid system: {12-column / split layout / bento / stacked ribbon 등}
- Wrapper or max-width rule: {container max-width를 px 또는 vw로 잠근다 — 예: max-width 1280px, side-padding 64px → mobile 24px. 형용사 크기 묘사로 끝내지 않는다.}
...
- Whitespace strategy: {section 수직 padding, column gap을 px 또는 spacing-scale 수치로 잠근다 — 예: section-y 120px, column-gap 32px. tight/generous 형용사로 끝내지 않는다.}
- Section rhythm: {major·minor section 전환 vertical padding 범위를 px 단위로 잠근다 — 예: major 160px, minor 80px}
- Persistent background layer: {page 전반에 반복되는 structural layer — 예: 12-column hairline 0.06 opacity overlay, noise grain 0.04 opacity. 없으면 명시적으로 "없음"으로 표기}
...
- Hero or primary entry block: {headline structure, support copy, media or stat treatment — column span, vertical anchor, min-height 또는 viewport-height 단위로 잠근다. 예: 2-column centred, min-height 100vh}
- Key content blocks: {feature grid, manifesto, comparison list, simulator, pricing, dashboard modules 등 relevant section anatomy — 각 block의 column count, row height 또는 padding band, alignment anchor를 명시. 형용사 구조 묘사로 끝내지 않는다.}
...
- Motion signature: {easing을 cubic-bezier 또는 named variant 수준으로 잠근다. duration은 ms/s 단위로 구체화한다 (예: 0.6s cubic-bezier(0.16, 1, 0.3, 1)). ease-out 같은 generic 키워드로 끝내지 않는다. reveal/hover behavior와 interaction tempo도 함께 명시한다.}
...
- Hard implementation locks: {execution에서 임의로 바꾸면 안 되는 값 목록 — 예: max-width 1280px, font-size hero 72px / body 16px, letter-spacing heading −0.02em, border-radius 2px, cubic-bezier(0.16, 1, 0.3, 1). 이 목록 밖 값은 개발자가 결정한다고 명시.}
