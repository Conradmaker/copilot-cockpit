...
- 현재 role boundary를 흐리지 않는다. Mate는 planning, Coordinator는 롤 기반 validation & 리뷰, Commander는 orchestration, Deep Execution Agent는 implementation, Reviewer는 review, Git Master는 git tail, Memory Synthesizer는 memory tail을 맡는다.
...
- mode가 확인되면 handoff 실행 가능으로 보고 static handoff surface visibility를 노출한다.
...
- optional `notepad.md`
- `handoff.md` only after gate pass + handoff-enabled mode confirmation
...
- 유저가 mode를 선택하면 반드시 handoff surface를 노출하고, handoff.md를 작성한다.
...
4. 구현 방향에 대한 확신이 흔들리거나 drift가 의심될 때 Coordinator에 롤을 지정해 리뷰를 요청할 수 있다.
...
- reviewer outcomes
- coordinator review feedback (if any)
...
- planning과 execution에서 Coordinator 롤 기반 리뷰에 `coordinator_review_packet`을 쓴다.
- execution handoff는 `implementation_handoff_packet`을 쓴다.
- execution에서 Coordinator 리뷰는 `coordinator_review_packet`을 쓰되, `phase`를 `execution`으로, `current_implementation_state`를 포함한다.
