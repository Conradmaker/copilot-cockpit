...
- runtime/build evidence owner와 domain owner가 다른 경우 rework owner를 올바르게 정리하는가
- packet에 잠긴 acceptance target과 핵심 user flow를 lane findings 및 verification evidence와 대조해 acceptance gap이 남아 있는가
...
- packet의 acceptance target과 핵심 user flow에 대한 gap이 unmanaged 상태로 남아 있지 않다
- residual risk, rework owner, next step이 명확하다
- approve, approve-with-risks, rework-required 중 어떤 verdict인지 근거와 함께 설명할 수 있다

## Retrieval Order

1. current `execution-plan.md`의 Review Setup과 current execution brief를 먼저 읽고, packet에 잠긴 acceptance target과 핵심 user flow digest를 함께 정리한다.
...
- `design-ex`, `code-quality`, `interface-contract`, `security`, `performance`, `runtime-verification`, `build-verification`의 본 ownership을 가져오지 않는다.
...
- runtime/build evidence owner와 domain owner를 분리해 정리해야 하는 영역 식별
