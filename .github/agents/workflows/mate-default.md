...
- Planning을 닫기 전까지 planning quality gate를 세워야 한다.
...
8. 초안이 새롭거나 크고 모호하거나 cross-functional하면 role별로 분리된 coordinator lane을 최소 2개 열어 동시에 병렬로 council review를 진행한다. 필요한 경우 Explore와 Librarian를 같은 wave에 붙여 quality lift를 높인다.
...
12. Coordinator가 반환한 `Scores`와 PRD의 `Quality Gate` section을 대조해 planning quality gate를 평가한다.
13. planning quality gate를 통과하면 PRD 내용을 요약해 user에게 간단히 설명한다.
14. Mate가 current `prd.md`, coordinator signal의 내용을 통해 downstream mode를 결정한다. downstream mode는 `디자인만`, `기술설계만`, `둘 다` 중 하나다.
15. auto-decision 결과가 `디자인만`이면 Designer를 연다. `기술설계만`이면 technical seed가 약한지 확인하고 필요하면 targeted clarification 또는 research 뒤 Architector를 연다. `둘 다`이면 Designer를 먼저 열어 design output을 확인한 뒤 technical seed와 architecture ambiguity를 다시 점검하고 Architector를 연다.
16. `prd.md`, `artifacts.md`, `design.md(optional)`, `technical.md(optional)`를 latest approved version으로 동기화하고 planning을 종료한다.
...
- Coordinator `Scores`와 PRD `Quality Gate`의 total score가 88 이상이다.
- critical blocker가 없다.
- downstream owner가 채팅을 다시 읽지 않고도 시작할 수 있을 만큼 approved `prd.md`가 준비되어 있다.

Mate는 passed planning quality gate 없이는 planning을 approved PRD 상태로 닫지 않는다.

## Approval And Downstream Trigger

- default mode의 downstream Definition으로 넘어가기 위한 최소 gate는 approved `prd.md`와 coordinator-reviewed quality gate pass다.
- 정리된 PRD 요약을 user에게 간단히 설명한 뒤 Mate는 downstream mode를 auto-decision 한다.
- downstream mode 선택지는 `디자인만`, `기술설계만`, `둘 다`다.
	- `디자인만`이면 Designer를 연다.
	- `기술설계만`이면 Architector를 연다.
	- `둘 다`이면 Designer를 먼저 완료한 뒤 Architector를 연다.
- `기술설계만`이 선택되었거나 `둘 다`의 technical lane을 열 차례인데 technical seed가 약하거나 architecture ambiguity가 남아 있으면, Mate는 clarification 또는 research lane을 먼저 다시 열고 그 뒤 Architector를 연다.
