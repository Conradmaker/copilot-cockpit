...
default mode는 `prd.md`와 `references.md`를 갱신하고, gate 통과 뒤 Mate가 auto-decision으로 relevant downstream lane을 연다.
...
13. Mate가 current `prd.md`, `references.md`, coordinator signal을 다시 검토해 downstream mode를 auto-decision 한다. downstream mode는 `디자인만`, `기술설계만`, `둘 다` 중 하나다.
...
16. 추가 refinement가 없으면 `prd.md`, `references.md`, `design.md(optional)`, `technical.md(optional)`를 latest approved version으로 동기화하고 planning을 종료한다.
