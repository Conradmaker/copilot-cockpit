...
15. auto-decision 결과가 `디자인만`이면 Designer를 연다. `기술설계만`이면 technical seed가 약한지 확인하고 필요하면 targeted clarification 또는 research 뒤 Architector를 연다. `둘 다`이면 두 Designer, Architector를 동시에 병렬로 연다.
...
- downstream mode 선택지는 `디자인만`, `기술설계만`, `둘 다`다.
	...
	- `둘 다`이면 Designer와 Architector를 동시에 병렬로 연다.
