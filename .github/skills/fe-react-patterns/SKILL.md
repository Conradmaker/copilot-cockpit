...
description: "React component architecture and composition patterns for app-level UI. Use when designing or refactoring components, local APIs, boolean props to variants, compound components, props drilling, state lifting, controlled/uncontrolled patterns, or wrapper components. Always consult this skill when the user asks to make a component, split a component, share state between siblings, or clean up a tangled component tree, even if architecture is not mentioned. For shared primitives use fe-ui-element-components. For accessibility use fe-a11y. For rendering optimization use fe-react-performance. Triggers on: component architecture, split component, compound components, boolean props, props drilling, state lifting, controlled component, wrapper component, 컴포넌트 설계, 컴포넌트 분리, 합성 패턴, 상태 끌어올리기, 제어 컴포넌트."
