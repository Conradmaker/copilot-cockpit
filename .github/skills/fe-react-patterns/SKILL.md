...
description: "React component architecture and composition patterns for app-level UI. Use this skill when designing or refactoring React components and local component APIs: splitting large components, replacing boolean props with explicit variants, building compound components, reducing props drilling, choosing controlled vs uncontrolled state, lifting shared state, or shaping wrapper components. Always consult this skill when the user asks to make a component, split a component, share state between siblings, or clean up a tangled component tree, even if they never mention architecture directly. Use fe-ui-element-components for shared design-system primitives, fe-a11y for accessibility, and fe-react-performance for rendering optimization. Triggers on: make a component, split component, component API, composition pattern, compound components, boolean props, render props, props drilling, state lifting, controlled component, uncontrolled component, wrapper component, shared state, 컴포넌트 설계, 컴포넌트 API, 합성 패턴, 컴파운드 컴포넌트, 컴포넌트 분리, props drilling, 상태 끌어올리기, 제어 컴포넌트, 비제어 컴포넌트, 상태 공유."
