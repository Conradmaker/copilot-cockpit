---
name: fe-a11y
description: "Frontend web accessibility patterns for interactive UI, semantic HTML, keyboard support, accessible names, ARIA state, focus management, and screen reader behavior. Use this skill when building or reviewing forms, modals, dialogs, tabs, accordions, switches, dropdowns, buttons, inputs, or shared UI primitives, or when working on ARIA, role/label/state wiring, semantic HTML, keyboard navigation, live regions, focus traps, or a11y linting. Always consult this skill for any interactive component or reusable wrapper, even if the user only asked to make a button, form, modal, tab, dropdown, or design-system primitive and never mentioned accessibility. For React component architecture use fe-react-patterns. For shared component API design use fe-ui-element-components. Triggers on: accessibility, a11y, ARIA, screen reader, keyboard navigation, focus management, semantic HTML, accessible name, role, aria-label, aria-expanded, live region, modal dialog, form accessibility, tab component, dropdown, shared UI component, reusable primitive, 접근성, 스크린리더, 키보드 네비게이션, 포커스 관리, 시맨틱 HTML, ARIA, 모달, 폼, 탭, 드롭다운, 다이얼로그, 버튼, 재사용 컴포넌트, 웹 접근성."
