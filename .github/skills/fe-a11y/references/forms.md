...
모바일에서 `autoFocus`는 페이지 진입 직후 가상 키보드를 열어 사용자의 맥락을 끊을 수 있다.

- `autoFocus`는 기본적으로 desktop-only로 본다.
- 검색이나 로그인처럼 사용자가 곧바로 입력하기를 기대하는 경우에만 제한적으로 사용한다.
- 여러 입력이 있는 화면에서는 단일 핵심 필드 하나에만 적용을 검토한다.
