---
name: fe-tailwindcss
description: "Tailwind CSS v4 styling patterns for utility-class UI, theme tokens, layout utilities, className merging, and dark mode. Use this skill for any Tailwind styling work—writing classes, defining cva variants, merging classNames with cn(), configuring @theme/@utility, responsive stacks, dark mode, or animations. Always consult this skill even if the user only mentions spacing, responsiveness, className conflicts, or style cleanup without explicitly saying 'Tailwind'. For typography decisions use ds-typography; for visual direction use ds-visual-design; for component APIs use fe-ui-element-components; for accessibility use fe-a11y. Triggers on: Tailwind CSS, className, cn(), cva, @theme, @utility, dark mode, responsive styling, 테일윈드, 클래스네임, 다크모드, 반응형, 스타일링."
---
