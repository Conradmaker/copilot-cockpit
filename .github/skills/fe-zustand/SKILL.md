---
name: fe-zustand
description: "Zustand v5 state management patterns for React applications, covering store creation, selectors, slices, middleware, persistence, Next.js and SSR boundaries, testing, and TanStack Query separation. Use this skill when building or refactoring client state with Zustand, choosing between create and createStore, fixing re-renders from broad subscriptions, composing persist/devtools/immer middleware, structuring slice-based stores, wiring scoped vanilla stores through context, or handling hydration issues in Next.js. Always consult this skill for Zustand work even if the user only asks to add a store, persist UI state, fix a Zustand bug, or decide whether state belongs in Zustand or TanStack Query. For component architecture use fe-react-patterns. For rendering optimization beyond store selection use fe-react-performance. For server-state caching and mutations use fe-tanstack-query. Triggers on: Zustand, zustand, createStore, createWithEqualityFn, useStoreWithEqualityFn, useShallow, persist middleware, devtools, subscribeWithSelector, slices pattern, client state, global store, vanilla store, hydration mismatch, Next.js Zustand, 전역 상태, 클라이언트 상태, 스토어 설계, 셀렉터 최적화, persist, 슬라이스 패턴, 하이드레이션, Zustand 버그."
