...
description: "Writing methods for social posts, blog posts, articles, and channel-specific content drafts. Use this skill when the user wants to draft, structure, rewrite, repurpose, or plan audience-facing content such as LinkedIn posts, X threads, founder posts, educational posts, blog drafts, article outlines, thought-leadership pieces, or content adapted across channels. Always consult this skill for content-format and voice decisions that affect hook, angle, structure, pacing, scannability, and CTA, even if the user only asks to 'write a post', 'turn this into a blog', or 'make this publishable'. For sentence-level clarity or AI-pattern cleanup use writing-clearly or humanizer-style skills. For product or technical docs such as README, changelog, and API reference use documentation-focused skills. For research and source gathering use research-focused skills. For keyword and search-intent optimization use seo-content-style skills. Triggers on: social post, LinkedIn post, X thread, founder post, blog post, article, content outline, repurpose content, thought leadership, content angle, publishable draft, 콘텐츠 작성, 링크드인 글, 소셜 글, 블로그 글, 아티클, 글 구조, 초안 작성, 발행용 글, 글감 정리."
...
이 스킬을 사용할 때는 글만 던지지 말고, 현재 판단한 구조와 이유를 함께 제시한다.
