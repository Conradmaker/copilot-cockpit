...
description: "React/Next.js performance optimization patterns prioritizing highest-impact fixes: async waterfalls, bundle size, server/RSC fetching, hydration, duplicate requests, re-renders. Use when optimizing slow pages, heavy apps, Suspense boundaries, lazy loading, code splitting, memoization, server components, server actions, or Core Web Vitals. Always consult this skill for React/Next.js performance work—even if the user only mentions 'slow', 'laggy', 'heavy', 'optimize', or 'bundle'. For component architecture use fe-react-patterns. For broad frontend audits use fe-code-review. Triggers on: React performance, Next.js performance, slow page, bundle size, hydration, async waterfall, Suspense, lazy loading, memo, useMemo, useCallback, RSC, server components, TTFB, LCP, Core Web Vitals, 성능 최적화, 느린 페이지, 번들, 하이드레이션, 워터폴, 리렌더링, 앱이 무거워."
