...
description: "Performance optimization patterns for React and Next.js focused on the highest-impact fixes first: async waterfalls, bundle size, server and RSC fetching, hydration cost, duplicate requests, and unnecessary re-renders. Use this skill when a React or Next.js page is slow, heavy, laggy, or expensive to load, or when optimizing Suspense, lazy loading, code splitting, memoization, server components, server actions, or Core Web Vitals. Always consult this skill for React or Next.js performance work even if the user only asks to speed up loading, reduce bundle weight, fix excessive re-renders, or find why the app feels heavy. For component architecture use fe-react-patterns. For broad frontend audits use fe-code-review. Triggers on: React performance, Next.js performance, slow page, initial load, bundle size, hydration, async waterfall, duplicate fetch, Suspense, lazy loading, code splitting, dynamic import, memo, useMemo, useCallback, RSC, server components, server actions, TTFB, LCP, Core Web Vitals, 성능 최적화, 느린 페이지, 초기 로딩, 번들, 하이드레이션, 워터폴, 중복 요청, 코드 스플리팅, 리렌더링, 앱이 무거워."
