---
name: ds-ui-patterns
description: "UI layout and section-composition patterns for landing pages, dashboards, AI product surfaces, component-level screen patterns, and presentation-style showcases. Use this skill when designing page structure, hero sections, bento grids, sidebars, pricing or analytics pages, navigation menus, portfolio showcases, or screen patterns such as cards, tab bars, carousels, overlays, and auth forms. Always consult this skill for layout, section rhythm, and surface composition decisions, even if the user only asked to make a landing page, build a dashboard, or polish a screen arrangement. For product-level UX flows and copy use ds-product-ux. For visual foundations use ds-visual-design. For Tailwind CSS implementation use fe-tailwindcss. For component API design use fe-ui-element-components. Triggers on: layout pattern, page layout, landing page, hero section, bento grid, dashboard layout, sidebar, pricing page, analytics page, navigation menu, card layout, tab bar, carousel, footer, overlay, login form, signup form, showcase, portfolio, mock-up, AI UI, 레이아웃, 페이지 구성, 랜딩 페이지, 히어로 섹션, 벤토 그리드, 대시보드, 사이드바, 가격 페이지, 분석 페이지, 내비게이션, 카드 레이아웃, 탭 바, 캐러셀, 푸터, 오버레이, 로그인 폼, 회원가입 폼, 쇼케이스, 포트폴리오, 목업, AI UI."
---
