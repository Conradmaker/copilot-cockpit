...
- centered poster hero는 안전한 기본값 중 하나일 뿐이다. 메시지와 media 구조에 따라 centered poster hero, asymmetrical split hero, full-bleed media hero, extended grid hero 중에서 고른다
...
- wide desktop(1440~1920px)은 별도 surface로 본다. page shell, content rail, prose measure, display measure, background/media bleed를 분리해 결정한다
...
- 1440px 이상에서 콘텐츠만 1200px 근처에 멈추고 배경선, 이미지, 장식 레이어가 따로 놀면 wide desktop 전략이 빠진 상태로 본다
- 한국어 hero headline이 desktop에서 4줄 이상으로 빨리 감기면 body prose measure를 재사용한 것인지 먼저 확인한다
...
현재 surface를 먼저 특정하고, 해당 행의 우선 가치를 기준으로 판단한다. 특히 랜딩 페이지와 editorial surface는 1440px 이상에서 별도의 wide desktop composition이 필요한지 항상 함께 검토한다.
