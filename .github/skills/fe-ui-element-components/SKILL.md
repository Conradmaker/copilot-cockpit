...
description: "UI element component authoring patterns for shared design systems. Use this skill when building reusable UI components that will be shared across projects, designing a component library, defining primitives vs components vs blocks, setting up design tokens and theme architecture, exposing styling/state contracts with data attributes, designing component APIs (as/asChild, prop type export, single element boundary), documenting shared components, or choosing how to distribute them via npm, registry, or marketplace. Always consult this skill when the component is intended for a shared design system, internal UI kit, component package, registry, or multi-project reuse, even if the user only asked to 'make a button'. Do not use this skill for one-off app screens or normal product UI implementation; use fe-react-patterns, fe-a11y, fe-code-conventions, and fe-react-performance there. For Tailwind CSS styling patterns (className merge, CVA, tokens, animations) use fe-tailwindcss. Triggers on: design system, ui kit, shared component library, primitive, token, theme, theming, registry, npm package, component docs, multi-project component, reusable UI kit, data-state, data-slot, component distribution, as prop, asChild, polymorphic component, prop type export, component API design, 디자인 시스템, UI 킷, 공용 컴포넌트, 공유 컴포넌트 라이브러리, 프리미티브, 디자인 토큰, 테마, 레지스트리, npm 배포, 컴포넌트 문서화, 공통 버튼, 멀티 프로젝트 재사용, 컴포넌트 API 설계, polymorphism."
---
