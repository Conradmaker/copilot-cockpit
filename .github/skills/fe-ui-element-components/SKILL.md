...
description: "Patterns for reusable design-system UI components and shared component libraries. Use this skill when building or reviewing shared primitives, buttons, dialogs, inputs, or other components meant for a design system, UI kit, npm package, registry, or multi-project reuse; deciding primitive vs component boundaries; defining tokens or theming; exposing stable styling contracts like data-state or data-slot; or shaping shared component APIs such as variants, asChild, and exported prop types. Always use it for shared UI work even if the user only asks to make or clean up a component. Do not use it for one-off product screens or local feature components; use fe-react-patterns, fe-a11y, fe-code-conventions, and fe-react-performance there. For Tailwind implementation use fe-tailwindcss. Triggers on: design system, UI kit, shared component, component library, primitive, design token, theming, data-state, data-slot, asChild, polymorphic component, prop type export, npm package, registry, reusable UI, 디자인 시스템, UI 킷, 공용 컴포넌트, 공유 컴포넌트 라이브러리, 프리미티브, 디자인 토큰, 컴포넌트 API."
