---
name: be-drizzle
description: "Drizzle ORM 스키마 설계, relation modeling, migrations, query optimization, transactions, connection management, drizzle-kit 설정을 위한 스킬이다. Use this skill when working on Drizzle schema files (pgTable, mysqlTable, sqliteTable), adding or refactoring Drizzle models, choosing relation patterns, wiring drizzle() instances, configuring PostgreSQL/MySQL/SQLite/Neon/Turso/PlanetScale, fixing migration drift, or optimizing queries. Always consult this skill for Drizzle-backed persistence work, even if the user only asks to 'add a table', 'connect the database', 'fix a Drizzle error', 'wire the client', or 'set up Drizzle'. For REST API contract design use be-api-design. For Prisma-backed persistence use be-prisma. For raw SQL query builder use be-kysely. Triggers on: Drizzle, drizzle-orm, drizzle-kit, drizzle.config, pgTable, mysqlTable, sqliteTable, relations(), $inferSelect, $inferInsert, drizzle-kit generate, drizzle-kit migrate, drizzle-kit push, drizzle-kit studio, node-postgres, postgres-js, Neon, Turso, PlanetScale, edge runtime, serverless database, 드리즐, 스키마 설계, 마이그레이션, DB 연결, ORM."
---
