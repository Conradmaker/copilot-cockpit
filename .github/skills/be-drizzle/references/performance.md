...
> **Note:** For Neon-specific HTTP adapter in Edge, see `references/neon.md`.
...
// Suitable for databases with HTTP query endpoints (e.g., Neon HTTP, Turso)
import postgres from 'postgres';
import { drizzle } from 'drizzle-orm/postgres-js';
...
// Each query is a single request
const users = await db.select().from(users);
```

> For Neon HTTP adapter specifically, see `references/neon.md`.
...
10. **Use HTTP-based drivers** (postgres-js, Neon HTTP) for edge/serverless
