...
description: Foundational research methodology for work that should be grounded in local rules, existing implementation patterns, official docs, and source-ladder evidence before moving into domain-specific execution. Use this skill when the user asks to research first, verify assumptions, check current rules, confirm behavior against docs, strengthen references, or avoid guesswork before planning, implementation, review, customization, or design decisions. Always consult this skill for evidence-first work that depends on codebase context plus external source-of-truth verification, even if the user only says to check official docs first or not to guess. For domain-specific research execution, hand off to research-product, research-design, research-content-source, and the relevant domain skills after the foundation pass. Triggers on: research-foundation, evidence pass, local rules first, official docs first, source ladder, verify assumptions, research first, stronger references, 근거 우선, 자료조사 방법론, 로컬 규칙 확인, 공식 문서 확인, 추측하지 말고 확인.
---
...
정확도가 중요한 작업에서 답을 먼저 만들지 말고, 기반 근거를 먼저 만든다.

이 스킬은 특정 주제의 facts를 저장해 두는 데이터베이스가 아니라, 자료조사를 어떻게 시작하고 어떤 기준으로 검증하고 언제 멈출지 정하는 foundation knowledge base다.
...
- 이 스킬은 자료조사 방법론, source ladder, foundation evidence pass, 근거 합성에 집중한다. 실제 구현 규칙은 해당 도메인 스킬을 이어서 읽는다.
- 이 스킬은 특정 도메인의 facts를 깊게 파는 owner가 아니다. 제품 리서치는 research-product, 디자인 레퍼런스 조사는 research-design, 콘텐츠 소스 조사는 research-content-source가 owner다.
