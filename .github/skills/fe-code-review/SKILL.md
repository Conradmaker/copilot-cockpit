---
name: fe-code-review
description: "Unified frontend review combining code conventions, React patterns, accessibility, and performance in one pass. Use this skill when reviewing PRs, diffs, changed files, or existing frontend code for bugs, regressions, code smells, accessibility issues, performance risks, or merge-readiness. Always consult this skill for any frontend review task, even if the user only asks 'look over this', 'find issues', 'check before merge', 'is this OK?', or 'what should I improve'. For writing or refactoring code, use fe-code-conventions, fe-react-patterns, fe-a11y, and fe-react-performance directly. Triggers on: code review, PR review, audit code, merge readiness, frontend review, 코드 리뷰, PR 리뷰, 머지 전 체크, 코드 검토, 머지해도 될까, 코드 봐줘."
