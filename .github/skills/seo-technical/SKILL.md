---
name: seo-technical
description: "Technical SEO implementation and audit patterns for crawlability, indexation, rendering, structured-data validation, redirects, Core Web Vitals, mobile parity, HTTPS, hreflang, and AI crawler access. Use this skill when the user asks why pages are not indexed, why Google, Bing, Copilot, Claude, or Perplexity cannot see a site, or how to audit robots.txt, sitemap.xml, canonicals, status codes, server errors, redirect chains, crawl budget, JavaScript rendering, schema markup, mixed content, page speed, mobile SEO, international SEO, IndexNow, or Bing and Brave visibility. Also use as entry point for broad SEO diagnosis when scope is unclear — audit first, then hand off to seo-content if title/meta/on-page issues dominate. Always consult this skill for site mechanics that decide whether search and AI engines can discover, render, validate, and trust pages, even if the request only mentions page speed, schema, or bot access. For title and meta copy, search intent, on-page scoring, snippets, and GEO writing use seo-content. Triggers on: technical SEO, indexing, crawlability, robots.txt, sitemap.xml, canonical, redirect chain, hreflang, structured data, Core Web Vitals, page speed, JavaScript SEO, AI bot access, 5xx, mixed content, crawl budget, SEO check, SEO help, SEO audit, why not ranking, traffic dropped, 테크니컬 SEO, 인덱싱, 크롤링, 로봇, 사이트맵, SEO 체크, SEO 진단, 랭킹 안 뜨, 유량 감소."
...
1. scope 요약
2. overall health 또는 핵심 blocking issue
3. findings table: issue / impact / evidence / fix / priority
...
### 3. migration 또는 launch checklist 요청
