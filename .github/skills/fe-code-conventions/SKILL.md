...
description: "Frontend maintainability principles for readability, predictability, cohesion, and coupling in application code. Use this skill when writing or refactoring frontend code and you need to improve naming, simplify long functions or complex conditionals, replace magic numbers, remove hidden side effects, normalize return shapes, choose file or folder placement, colocate related code, or decide whether shared logic should be abstracted or kept duplicated locally. Always consult this skill for frontend cleanup and structure decisions, even if the user only says 'clean this up', 'this function is too long', 'this condition is hard to read', 'where should this code live?', or 'should we extract this?'. For React-specific component architecture use fe-react-patterns. For broad frontend code review use fe-code-review. Triggers on: clean code, code refactoring, naming, readability, predictability, cohesion, coupling, long function, complex conditional, nested ternary, magic number, hidden side effect, return type consistency, file structure, folder structure, colocation, abstraction, duplication, code organization, 코드 컨벤션, 클린코드, 리팩토링, 가독성, 예측 가능성, 응집도, 결합도, 변수명, 함수명, 폴더 구조, 디렉토리 구조, 코드 정리, 코드 개선, 매직 넘버, 하드코딩, 조건문 정리, 중첩 삼항, 숨은 로직, 사이드 이펙트, 함수가 너무 길어, 이름이 애매해, 어디에 둬야 해, 공통화, 중복 허용, 유지보수."
