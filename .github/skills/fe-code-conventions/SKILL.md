...
description: "Maintainable frontend code for readability, predictability, cohesion, and coupling. Use this skill when refactoring frontend code with unclear naming, long functions, complex conditionals, nested ternaries, magic numbers, hidden side effects, inconsistent return shapes, weak colocation, or unclear abstraction vs duplication choices. Always consult it for frontend cleanup and code organization, even if the user only says 'clean this up', 'this function is too long', 'this condition is hard to read', 'where should this live?', or 'should we extract this?'. For React architecture use fe-react-patterns. For broad review use fe-code-review. Triggers on: clean code, frontend refactor, naming, readability, predictability, cohesion, coupling, long function, complex conditional, nested ternary, magic number, hidden side effect, file structure, colocation, abstraction, duplication, 코드 컨벤션, 클린코드, 리팩토링, 가독성, 예측 가능성, 응집도, 결합도, 폴더 구조, 코드 정리, 공통화."
