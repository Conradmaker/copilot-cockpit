...
description: "제품·기술 문서 작성 패턴을 다루는 스킬이다. Changelog, release notes, API reference, technical guide(How-to, tutorial, getting started, quickstart), migration guide, Architecture Decision Record(ADR), configuration reference, SDK documentation, troubleshooting guide, internal technical spec 같은 제품·개발 문서를 쓰거나 리뷰할 때 사용한다. Always consult this skill for product or technical documentation work, even if the user only asks to 'write release notes', 'document this API', 'add a changelog entry', 'create a migration guide', 'write a getting started page', or 'document this config'. For sentence-level clarity, AI-pattern removal, or prose polish use writing-clearly. For audience-facing content such as blog posts, social posts, or articles use writing-content. For README files use writing-readme. For UX copy, error messages, or CTA labels use ds-product-ux. Triggers on: changelog, release notes, API reference, API documentation, technical guide, how-to, tutorial, getting started, quickstart, migration guide, breaking change, ADR, architecture decision record, configuration reference, SDK documentation, troubleshooting guide, technical spec, developer documentation, OpenAPI, version history, deprecation notice, 체인지로그, 릴리스 노트, API 문서, 기술 문서, 가이드 작성, 마이그레이션 가이드, 설정 문서, SDK 문서, 트러블슈팅, 기술 명세, 개발 문서."
---
